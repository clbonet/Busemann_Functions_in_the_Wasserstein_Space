...
def sliced_wasserstein2(
    ...
    # slice source and targets along the directions, shape (n_projs, n_samples)
    proj_target = (target @ directions.T).T
    proj_source = (source @ directions.T).T

    proj_target_sorted = jnp.sort(proj_target, axis=1)
    proj_source_sorted = jnp.sort(proj_source, axis=1)

    diff = norm_1d(proj_target_sorted - proj_source_sorted)
